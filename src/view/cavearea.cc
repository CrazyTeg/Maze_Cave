...
void CaveArea::drawFromFile(QString filename) {
    controller_->cave()->readFile(filename.toStdString());
    drawCave();
}

void CaveArea::drawGenerate(int width, int height, int chance) {
    controller_->cave()->generateMap(chance, width, height);
    drawCave();
}
...
void CaveArea::drawCave() {
    ...
    cellWidth_ = 500.0 / controller_->cave()->getCols();
    cellHeight_ = 500.0 / controller_->cave()->getRows();
    for (int i = 0; i < controller_->cave()->getRows(); i++) {
        for (int j = 0; j < controller_->cave()->getCols(); j++) {
            if (controller_->cave()->map()(i, j)) {
                painter.drawRect(
